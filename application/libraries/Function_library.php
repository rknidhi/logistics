...
class Function_library {
    ...
    public function FindVehicle1($vehicle_id) {
        return $this->obj->functions->get_single_row_colum('tbl_master_vehicle', 'fleet_no', 'vehicle_id', $vehicle_id);
    }
   ...
   // Edited by Rakesh
    ...
    public function getVechileList(){
        ...
        $grValues1 = array_filter($grValues1);
        }
        else {
            $grValues1[] =  '';
            $grValues1[] =  '';
    ...
    foreach($resultset as $row){
        ...
        $grlist .= '<tr><td>'.$srno.'<td>'.$row->bgr_id.'</td><td>'. $this->obj->function_library->FindPartyDetails($row->consignor_id_fk)->party_name.'</td><td>'.$this->obj->function_library->FindPartyDetails($row->consignee_id_fk)->party_name.'</td><td>'.$row->invoice_no.'</td><td>'.$count_items.'</td><td>'.number_format($row->s_total_freight,2).'</td><td>'.$item_weight.'</td></tr>'."\n";
        $srno++;   
    }
...
public function getBillAmount($item_id)
 {
    return $this->obj->functions->get_sum_of_column('book_tr', 'bill_total' ,'bill_number', $item_id);
 }
 function getLHC($lhcid){
    $user = $this->obj->functions->get_single_row('challan_dispatch', 'cdispatch_id', $lhcid);
    return $user->lhc_number;
 }



 public function calculatePaymentreceived($billID, $type='', $vtype='') {
	 $this->obj->db->select_sum('amount');
        $this->obj->db->where_in('reference_no', $billID);
        if(!empty($type)){
            $this->obj->db->where_in('billtype', $type);
        }
		         if(!empty($vtype)){
            $this->obj->db->where('voucher_type', $vtype);
        }
        $result = $this->obj->db->get('payment_history')->row()->amount;
        //echo $this->obj->db->last_query();
        return $result;
    }
...
//    public function calculateDrFinal($group, $from_date, $to_date, $company, $branch, $voucher_type) {
    public function calculateDrFinal($group, $from_date, $to_date, $company, $branch) {        
        //return "$group, $from_date, $to_date, $company, $branch";
        ...
        //$this->obj->db->where('voucher_type', $voucher_type); // Special for profit and loss calculation
        
        return $this->obj->db->from('tbl_vouchers')->get()->row()->amount;
    }

  public function getVoucherParticular($args){
    $vt = '';
    if($args['vtype'] == 'Debit')
        $vt = 'Credit';
    if($args['vtype'] == 'Credit')
        $vt = 'Debit';
     
     $whQuery['voucher_type'] = $args['vt_type'];    
     $whQuery['voucher_no'] = $args['vno'];    
     $whQuery['transaction_type'] = $vt;              
      $res = $this->obj->db->where($whQuery)->get('tbl_vouchers')->result();  
    return $res;
   } 
    // End
}