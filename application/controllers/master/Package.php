...
class Package extends CI_Controller {
    ...
    function index() {
        if (!$this->flexi_auth->is_privileged('Packing Master')) {
        ...
        $this->load->view('master/package/view', $this->data);
    }

    function add_package() {
        $this->load->view('master/package/add', $this->data);
    }

    function edit_package($package_id) {
        $this->data['package'] = $this->functions->get_single_row('tbl_master_package', 'package_id', $package_id);
        $this->load->view('master/package/edit', $this->data);
    }

    function delete() {
        $package_id = $this->input->post('id');
        $response = $this->functions->delete_single_row('tbl_master_package', 'package_id', $package_id);
         //FOR ACTIVIES LOG
        $activites['user_id'] = $this->flexi_auth->get_user_id();
        $activites['activity_id'] = $package_id;
        $activites['activity_type'] = 'Package';
        $activites['action'] = 'deleted';
        $activites['name'] = $this->function_library->get_user_name($this->flexi_auth->get_user_id()) . ' deleted package.';
        $activites['module'] = 'master/package';
    ...
    function add() {
        ...
        $response = $this->functions->insert_into_table('tbl_master_package', $data);
        //FOR ACTIVIES LOG
        $activites['user_id'] = $this->flexi_auth->get_user_id();
        $activites['activity_id'] = $response;
        $activites['activity_type'] = 'Package';
        $activites['action'] = 'added';
        $activites['name'] = $this->function_library->get_user_name($this->flexi_auth->get_user_id()) . ' added package.';
        $activites['module'] = 'master/package';
        $activites['created_at'] = date("Y-m-d h:i:s");
        $this->functions->insert_into_table('activities', $activites);
    ...
    function update($package_id) {
        $data = $this->input->post('data');
        $data['modified_by'] = $this->flexi_auth->get_user_id();
        $response = $this->functions->update_table_rows('tbl_master_package', 'package_id', $package_id, $data);
        //FOR ACTIVIES LOG
        $activites['user_id'] = $this->flexi_auth->get_user_id();
        $activites['activity_id'] = $package_id;
        $activites['activity_type'] = 'Package';
        $activites['action'] = 'updated';
        $activites['name'] = $this->function_library->get_user_name($this->flexi_auth->get_user_id()) . ' updated package.';
        $activites['module'] = 'master/package';
        $activites['created_at'] = date("Y-m-d h:i:s");
        $this->functions->insert_into_table('activities', $activites);
        if ($response) {
