...
class Route extends CI_Controller {
    ...
    function index() {
        if (!$this->flexi_auth->is_privileged('Route Master')) {
        ...
        $this->data['routes'] = $this->functions->get_all_row('tbl_master_route');
        $this->load->view('master/route/view', $this->data);
    }
    ...
    function add_route() {
        ...
        $this->load->view('master/route/add', $this->data);
    }

    function edit_route($route_id) {
        $this->data['bagents'] = $this->functions->get_all_active_row('tbl_master_branch_agent');
        $this->data['stations'] = $this->functions->get_all_active_row('tbl_master_station');
        $this->data['route'] = $this->functions->get_single_row('tbl_master_route', 'route_id', $route_id);
        $this->load->view('master/route/edit', $this->data);
    }

    function detail_route($route_id) {
        $this->data['route'] = $this->functions->get_single_row('tbl_master_route', 'route_id', $route_id);
        $this->load->view('master/route/details', $this->data);
    }

    function delete() {
        $route_id = $this->input->post('id');
        $response = $this->functions->delete_single_row('tbl_master_route', 'route_id', $route_id);
         //FOR ACTIVIES LOG
        $activites['user_id'] = $this->flexi_auth->get_user_id();
        $activites['activity_id'] = $route_id;
        $activites['activity_type'] = 'Route';
        $activites['action'] = 'deleted';
        $activites['name'] = $this->function_library->get_user_name($this->flexi_auth->get_user_id()) . ' deleted route.';
        $activites['module'] = 'master/route';
    ...
    function add() {
        ...
        $response = $this->functions->insert_into_table('tbl_master_route', $data);
        //FOR ACTIVIES LOG
        $activites['user_id'] = $this->flexi_auth->get_user_id();
        $activites['activity_id'] = $response;
        $activites['activity_type'] = 'Route';
        $activites['action'] = 'added';
        $activites['name'] = $this->function_library->get_user_name($this->flexi_auth->get_user_id()) . ' added route.';
        $activites['module'] = 'master/route';
        $activites['created_at'] = date("Y-m-d h:i:s");
        $this->functions->insert_into_table('activities', $activites);
    ...
    function update($route_id) {
        $data = $this->input->post('data');
        $data['modified_by'] = $this->flexi_auth->get_user_id();
        $response = $this->functions->update_table_rows('tbl_master_route', 'route_id', $route_id, $data);
         //FOR ACTIVIES LOG
        $activites['user_id'] = $this->flexi_auth->get_user_id();
        $activites['activity_id'] = $route_id;
        $activites['activity_type'] = 'Route';
        $activites['action'] = 'updated';
        $activites['name'] = $this->function_library->get_user_name($this->flexi_auth->get_user_id()) . ' updated route.';
        $activites['module'] = 'master/route';
        $activites['created_at'] = date("Y-m-d h:i:s");
        $this->functions->insert_into_table('activities', $activites);
        if ($response) {
