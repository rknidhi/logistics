...
class Broker extends CI_Controller {
    ...
    function index() {
        if (!$this->flexi_auth->is_privileged('Broker Master')) {
        ...
        $this->load->view('master/broker/view', $this->data);
    }
    ...
    function add_broker() {
        ...
        $this->load->view('master/broker/add', $this->data);
    }
    ...
    function edit_broker($broker_id) {
        ...
        $this->data['bagents'] = $this->functions->get_all_active_row('tbl_master_branch_agent');
        $this->data['companies'] = $this->functions->get_all_active_row('tbl_master_company');
        $this->data['broker'] = $this->functions->get_single_row('tbl_master_broker', 'broker_id', $broker_id);
        $this->load->view('master/broker/edit', $this->data);
    }
    ...
    function broker_details($broker_id) {
        ...
        #$this->data['companies'] = $this->functions->get_all_active_row('tbl_master_company');
        $this->data['broker'] = $this->functions->get_single_row('tbl_master_broker', 'broker_id', $broker_id);
        $this->load->view('master/broker/details', $this->data);
    }

    function add_broker_ajax() {
        $this->data['freight_methods'] = $this->functions->get_all_active_row('tbl_master_freight_method');
        $this->data['bagents'] = $this->functions->get_all_active_row('tbl_master_branch_agent');
        $this->data['companies'] = $this->functions->get_all_active_row('tbl_master_company');
        $this->load->view('master/broker/add_ajax', $this->data);
    }
    ...
    function add_ajax() {
        ...
        $response = $this->functions->insert_into_table('tbl_master_broker', $data);

        //Added In Ledger Master
        $ledgerdata['ledger_type'] = 3; //Broker
        $ledgerdata['reference_no'] = $response;
        $ledgerdata['ledger_name'] = $data['broker'];
        $ledgerdata['ledger_tin'] = '';
        $ledgerdata['gstin'] = $data['service_tax_no'];
       ...
       // $ledgerdata['company_id_fk'] = $data['company_id_fk'];
        ...
        $activites['activity_type'] = 'Broker';
        $activites['action'] = 'added';
        $activites['name'] = $this->function_library->get_user_name($this->flexi_auth->get_user_id()) . ' added broker.';
        $activites['module'] = 'master/broker';
    ...
    function delete() {
        $broker_id = $this->input->post('id');
        $response = $this->functions->delete_single_row('tbl_master_broker', 'broker_id', $broker_id);
        //FOR ACTIVIES LOG
        $activites['user_id'] = $this->flexi_auth->get_user_id();
        $activites['activity_id'] = $broker_id;
        $activites['activity_type'] = 'Broker';
        $activites['action'] = 'deleted';
        $activites['name'] = $this->function_library->get_user_name($this->flexi_auth->get_user_id()) . ' deleted broker.';
        $activites['module'] = 'master/broker';
        $activites['created_at'] = date("Y-m-d h:i:s");
        $this->functions->insert_into_table('activities', $activites);
        if ($response) {
    ...
    function update($broker_id) {
        $data = $this->input->post('data');
        $data['modified_by'] = $this->flexi_auth->get_user_id();
        $response = $this->functions->update_table_rows('tbl_master_broker', 'broker_id', $broker_id, $data);
        //FOR ACTIVIES LOG
        $activites['user_id'] = $this->flexi_auth->get_user_id();
        $activites['activity_id'] = $broker_id;
        $activites['activity_type'] = 'Broker';
        $activites['action'] = 'updated';
        $activites['name'] = $this->function_library->get_user_name($this->flexi_auth->get_user_id()) . ' updated broker.';
        $activites['module'] = 'master/broker';
        $activites['created_at'] = date("Y-m-d h:i:s");
        $this->functions->insert_into_table('activities', $activites);
        if ($response) {
