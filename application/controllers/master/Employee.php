...
class Employee extends CI_Controller {
    ...
    function index() {
        if (!$this->flexi_auth->is_privileged('Employee Master')) {
        ...
        $this->load->view('master/employee/view', $this->data);
    }
    ...
    function add_employee() {
        ...
        $this->load->view('master/employee/add', $this->data);
    }

    function edit_employee($employee_id) {
        $this->data['bagents'] = $this->functions->get_all_active_row('tbl_master_branch_agent');
        $this->data['companies'] = $this->functions->get_all_active_row('tbl_master_company');
        $this->data['employee'] = $this->functions->get_single_row('tbl_master_employee', 'employee_id', $employee_id);
        $this->load->view('master/employee/edit', $this->data);
    }
    ...
    function employee_details($employee_id) {
        ...
        #$this->data['companies'] = $this->functions->get_all_active_row('tbl_master_company');
        $this->data['employee'] = $this->functions->get_single_row('tbl_master_employee', 'employee_id', $employee_id);
        $this->load->view('master/employee/details', $this->data);
    }

    function delete() {
        $employee_id = $this->input->post('id');
        $response = $this->functions->delete_single_row('tbl_master_employee', 'employee_id', $employee_id);
        //FOR ACTIVIES LOG
        $activites['user_id'] = $this->flexi_auth->get_user_id();
        $activites['activity_id'] = $employee_id;
        $activites['activity_type'] = 'Employee';
        $activites['action'] = 'deleted';
        $activites['name'] = $this->function_library->get_user_name($this->flexi_auth->get_user_id()) . ' deleted employee.';
        $activites['module'] = 'master/employee';
    ...
    function add() {
        ...
        $response = $this->functions->insert_into_table('tbl_master_employee', $data);

        //Added In Ledger Master
        $ledgerdata['ledger_type'] = 5; //Employee
        $ledgerdata['reference_no'] = $response;
        $ledgerdata['ledger_name'] = $data['employee_name'];
        ...
        $activites['name'] = $this->function_library->get_user_name($this->flexi_auth->get_user_id()) . ' added employee.';
        $activites['module'] = 'master/employee';
        $activites['created_at'] = date("Y-m-d h:i:s");
        $this->functions->insert_into_table('activities', $activites);
    ...
    function update($employee_id) {
        $data = $this->input->post('data');
        $data['modified_by'] = $this->flexi_auth->get_user_id();
        $response = $this->functions->update_table_rows('tbl_master_employee', 'employee_id', $employee_id, $data);
        //FOR ACTIVIES LOG
        $activites['user_id'] = $this->flexi_auth->get_user_id();
        $activites['activity_id'] = $employee_id;
        $activites['activity_type'] = 'Employee';
        $activites['action'] = 'updated';
        $activites['name'] = $this->function_library->get_user_name($this->flexi_auth->get_user_id()) . ' updated employee.';
        $activites['module'] = 'master/employee';
        $activites['created_at'] = date("Y-m-d h:i:s");
        $this->functions->insert_into_table('activities', $activites);
        if ($response) {
