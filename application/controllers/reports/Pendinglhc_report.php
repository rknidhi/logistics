...
class Pendinglhc_report extends CI_Controller {
    ...
    function index() {
        if (!$this->flexi_auth->is_privileged('Booking Report')) {
        ...
        $lhcno = $this->db->get('challan_dispatch')->result();
        foreach($lhcno as $row){
            $lhcnos[] = $row->gr_no_fk.','.$row->gr_no_inc;
        }
        $lhcnos = implode(',',$lhcnos);
        $lhcnos = explode(',',$lhcnos);
        $lhcnos = array_unique(array_filter($lhcnos));
        ...
        if (!empty($this->input->post('submit'))) {
            ...
            if (!empty($filter['item_name_fk'])) {
                $change_view = TRUE;
                $item_list = array();
                $item_list = $this->functions->get_all_row_id_based('tbl_gr_items', 'item_name_fk', $filter['item_name_fk']);
                    $this->data['item_name'] = $item_list;
            }
            ...
            $this->db->select('*');
            $this->db->where_not_in('bgr_id',$lhcnos);
            ...
            if (!empty($filter['gr_from'])) {
                $this->db->where('bgr_id >=', $filter['gr_from']);
            }
            if (!empty($filter['gr_to'])) {
                $this->db->where('bgr_id <=', $filter['gr_to']);
            }
...
/*            if (!empty($filter['item_remarks'])) {
                $this->db->where('item_remarks', $filter['item_remarks']);
            }*/
            if (!empty($filter['vehicle_id_fk'])) {
                $this->db->where('vehicle_id_fk', $filter['vehicle_id_fk']);
            }
            if (!empty($filter['branch_id_fk'])) {
                $this->db->where('branch_id_fk', $filter['branch_id_fk']);
            }
            if (!empty($filter['gr_type'])) {
                $this->db->where('gr_type', $filter['gr_type']);
            }
        ...
        if($change_view){
            $this->load->view('reports/pendinglhc_report_01', $this->data);
        }
        else {
            $this->load->view('reports/pendinglhc_report', $this->data);
        }    
    ...
    function viewItems($gr_no)
    {
        ...
        $this->load->view('reports/gr_items_details', $data);
    }
    // End
}
