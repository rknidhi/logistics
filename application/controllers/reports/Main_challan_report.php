...
class Main_challan_report extends CI_Controller {
    ...
    function index() {
        if (!$this->flexi_auth->is_privileged('Main Challan Report')) {
        ...
        if (!empty($this->input->post('submit'))) {
            ...
            if (!empty($filter['challan_number_from'])) {
                $this->db->where('main_challan.challan_number >=', $filter['challan_number_from']);
            }
            if (!empty($filter['challan_number_to'])) {
                $this->db->where('main_challan.challan_number <=', $filter['challan_number_to']);
            }

            if (!empty($filter['bgr_id_fk'])) {
                $this->db->where('main_challan.bgr_id_fk', $filter['bgr_id_fk']);
            }

            if (!empty($filter['broker_id_fk'])) {
                $this->db->where('booking_gr.broker_id_fk', $filter['broker_id_fk']);
            }
            if (!empty($filter['driver_id_fk'])) {
                $this->db->where('challan_dispatch.driver_id_fk', $filter['driver_id_fk']);
            }
            if (!empty($filter['vehicle'])) {
                $this->db->where('booking_gr.vehicle_id_fk', $filter['vehicle']);
            }
            ...
            $this->db->join('challan_dispatch', "challan_dispatch.gr_no_fk = main_challan.bgr_id_fk");
        ...
        $this->load->view('reports/main_challan_report', $this->data);
    }
