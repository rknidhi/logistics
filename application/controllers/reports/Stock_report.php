...
class Stock_report extends CI_Controller {
    ...
    function index() {
        if (!$this->flexi_auth->is_privileged('Stock Report')) {
        ...
        if (!empty($this->input->post('submit'))) {
            ...
            if (!empty($filter['item_id_fk'])) {
                $this->db->where('tbl_warehouse.item_id_fk', $filter['item_id_fk']);
            }

            if (!empty($filter['branch_id_fk'])) {
                $this->db->where('tbl_warehouse.branch_id_fk', $filter['branch_id_fk']);
            }


            $this->data['records'] = $this->db->order_by('warehouse_id', 'DESC')->get('tbl_warehouse')->result();
