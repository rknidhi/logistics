...
class Unloading_report extends CI_Controller {
    ...
    function index() {
        if (!$this->flexi_auth->is_privileged('Unloading Report')) {
        ...
        if (!empty($this->input->post('submit'))) {
            ...
            if (!empty($filter['vehicle_id_fk'])) {
                $this->db->where('booking_gr.vehicle_id_fk', $filter['vehicle_id_fk']);
            }
            if (!empty($filter['consignor_id_fk'])) {
                $this->db->where('booking_gr.consignor_id_fk', $filter['consignor_id_fk']);
            }
            if (!empty($filter['consignee_id_fk'])) {
                $this->db->where('booking_gr.consignee_id_fk', $filter['consignee_id_fk']);
            }
            if (!empty($filter['item_name_fk'])) {
                $this->db->where('booking_gr.item_name_fk', $filter['item_name_fk']);
            }
        ...
        $this->data['routes'] = $this->functions->get_all_active_row('tbl_master_route');
        ...
        $this->load->view('reports/unloading_report', $this->data);
    }
