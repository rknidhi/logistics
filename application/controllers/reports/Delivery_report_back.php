...
class Delivery_report extends CI_Controller {
    ...
    function index() {
        if (!$this->flexi_auth->is_privileged('Delivery Report')) {
...
/*            if (!empty($filter['from_date'])) {
            ...
            if (!empty($filter['from_date'])) {
                $this->db->where('delivery_received_date >=', convertToMySQLDate($filter['from_date']));
            }
            if (!empty($filter['to_date'])) {
                $this->db->where('delivery_received_date <=', convertToMySQLDate($filter['to_date']));
            }
            // End
...
/*            if (!empty($filter['delivery_mode_fk'])) {
        ...
        $this->load->view('reports/delivery_report', $this->data);
    }
