...
class Pod_report extends CI_Controller {
    ...
    function index() {
        if (!$this->flexi_auth->is_privileged('POD Report')) {
        ...
        if (!empty($this->input->post('submit'))) {
            ...
            if (!empty($filter['pod_status'])) {
                $this->db->where('pod_status', $filter['pod_status']);
            }
            if (!empty($filter['pod_received_from'])) {
                $this->db->where('pod_received_date >=', convertToMySQLDate($filter['pod_received_from']));
            }
            if (!empty($filter['pod_received_to'])) {
                $this->db->where('pod_received_date <=', convertToMySQLDate($filter['pod_received_to']));
            }
        ...
        $this->load->view('reports/pod_report', $this->data);
    }
