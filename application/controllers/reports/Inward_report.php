...
class Inward_report extends CI_Controller {
    ...
    function index() {
        if (!$this->flexi_auth->is_privileged('Inward Report')) {
        ...
        if (!empty($this->input->post('submit'))) {
            ...
            $this->db->select('*');
            $this->db->from('tbl_inwards');

            if (!empty($filter['from_date'])) {
                $this->db->where('inward_date >=', convertToMySQLDate($filter['from_date']));
            }
            if (!empty($filter['to_date'])) {
                $this->db->where('inward_date <=', convertToMySQLDate($filter['to_date']));
            }
            if (!empty($filter['inward_no_from'])) {
                $this->db->where('inward_id >=', $filter['inward_no_from']);
            }
            if (!empty($filter['inward_no_to'])) {
                $this->db->where('inward_id <=', $filter['inward_no_to']);
            }
        ...
        $this->data['inwardtransport'] = $this->functions->get_transport_name('tbl_inwards');
        // End
        ...
        $this->load->view('reports/inward_report', $this->data);
    }
