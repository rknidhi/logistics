...
class Gst_report extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->vars('menu_nevigation', 'report');
    ...
    function index() {
        if (!$this->flexi_auth->is_privileged('GST Report')) {
        ...
        if (!empty($this->input->post('submit'))) {
            ...
            $this->db->select('book_billing.*');
            $this->db->from('book_billing');
            $this->db->join('tbl_master_party', "tbl_master_party.party_id = book_billing.bill_to_id","LEFT");

            if (!empty($filter['branch_id_fk'])) {
                $this->db->where('tbl_master_party.branch_agent_id_fk', $filter['branch_id_fk']);
            }
            if (!empty($filter['from_date'])) {
                $this->db->where('book_billing.invoice_date >=', convertToMySQLDate($filter['from_date']));
            }
            if (!empty($filter['to_date'])) {
                $this->db->where('book_billing.invoice_date <=', convertToMySQLDate($filter['to_date']));
            }
