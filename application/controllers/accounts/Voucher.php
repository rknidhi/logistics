    ...
    function index() {
        ...
        if (!empty($this->input->post('submit'))) {
           ...
           // print_r($this->input->post()); die;
            ...
            if($vtype == 'Payment' || $vtype == 'Journal'){
                $dr_data['transaction_type'] = 'Credit';
            }
            else{
            ...
            if($vtype == 'Payment' || $vtype == 'Journal'){
                $cr_data['transaction_type'] = 'Debit';
            }
            else{
            ...
            $cr_data['gr_no_disp'] = $data['gr_no_disp'];            
...
/*             $dr_data = $this->input->post('dr_data');
            ...
            foreach($historydata as $innerA){
                $innerA = json_decode(json_encode($innerA),true);
                $whereclouse = array('billtype'=>$vtype, 'voucher_type'=>$vtype,'voucher_no'=>$vno);
    ...
    function fetch_lhc_details(){
        ...
        $this->data['a'] = $grslist = $this->functions->get_single_row('challan_dispatch','cdispatch_id', $lhcno);
       // $grslist = $this->functions->get_single_row('challan_dispatch','cdispatch_id', $lhcno);
        $whq = array('reference_no'=>$lhcno,'billtype'=>'Challan');
        $this->data['received_amount'] = $this->functions->get_sum_of_pay_history('payment_history', 'amount', $whq);
        
        if($grslist->gr_no_inc !='') {
               $grlist1 = $grslist->gr_no_fk.','. $grslist->gr_no_inc;
        } else {
            $grlist1 = $grslist->gr_no_fk;            
        }

        $grlist1 = explode(',', $grlist1);
       
        $opt = '';
        foreach($grlist1 as $key => $value){
    ...
    function fetch_lhc_detailsj(){
        $lhcno = $this->input->post('id');
        $this->data['a'] = $this->functions->get_single_row('challan_dispatch','cdispatch_id', $lhcno);
        $whq = array('reference_no'=>$lhcno,'billtype'=>'Challan');
        $this->data['received_amount'] = $this->functions->get_sum_of_pay_history('payment_history', 'amount', $whq);
        echo json_encode($this->data);
    }
    ...
    function add_lhcaj($branchID, $ledg) {
        $findid = $this->functions->get_single_row('ac_ledger_master','ledger_id', $ledg);        
        $lhclist =  $this->functions->get_all_row_id_based('challan_dispatch','broker_id_fk', $findid->reference_no);
        $opt = '';
        foreach($lhclist as $lhc){
        ...
        $this->data['lhcnos'] = $opt;
        $this->load->view('account/voucher/add_lhcaj', $this->data);
    }


    function add_gra($branchID, $ledg) {
    ...
    function add_lhca_ajax(){
        ...
        $gr_index = 0;
        $temp_gr = array();
        for($grindex = 1; $grindex <= 5; $grindex++){
            foreach($ref_no['grnos'][$grindex] as $key => $value){
               if($value != ''){
                    $valdata['amount'] = $ref_no['amount'][$grindex-1];
                    $valdata['reference_no'] = $value;
                    $valdata['billtype'] = 'Gr';
                    $total_amount += $ref_no['amount'][$key];
                    $valdata1[] = json_encode($valdata);
        ...
        foreach($ref_no['lhcno'] as $key => $value){
             if($value != ''){
                $valdata['link_gr_no'] = $temp_gr[$grindex]; 
                $valdata['amount'] = $ref_no['amount'][$key];
                $valdata['reference_no'] = $value;
    ...
    function add_gr_ajax(){
        ...
        $refno1 = array_unique($refno, SORT_NUMERIC);
        $return_ref_no = implode(',', $refno1);
        $ajaxdata['gr_no_visible'] = $return_ref_no; 
        $ajaxdata['payhistory'] = str_replace('\\','',json_encode($valdata1));
        $ajaxdata['status'] = 'success';
    ...
    function edit_bia($branch, $vtype, $vno)
    {
        ...
        $grnos_disp = explode(',', $redno->gr_no_disp);
        $data['grnos'] = $grnos;
        $data['grnos_disp'] = $grnos_disp;

        $data['voucher_no'] = $vno;
        $data['voucher_type'] = $vtype;

        $data['hisrec'] = $historyrec = $this->db->select('*')
                                ->where_in('reference_no', $grnos)->where('voucher_no', $vno)
