...
class Auth_admin extends CI_Controller {
     ...
     */
    function dashboard() {
        // Modified by Rakesh Dated: 30-10-19
            //Total GR
            $this->db->select('*');
            ...
            $record = $this->db->get('challan_dispatch')->result();
            foreach($record as $row){
                $grno[] = $row->gr_no_fk.','.$row->gr_no_inc;
            }
            if(!empty($grno)){
                $grnoAdded1 = explode(',',implode(',',$grno));
                $grnoAdded1= array_unique(array_filter($grnoAdded1));
                $this->data['total_lhc'] = count($grnoAdded1); 
            }
            //TOTAL POD RECEIVED
            $this->db->select('*');
            $this->db->from('booking_gr');
...
//            $this->db->where('bill_number !=', 0);
            ...
            $gr_total = $this->db->get()->row()->amount;
            ...
            $rec = $this->db->select('*')
                            ...
                            foreach($rec->result() as $row){
                                $res[] = $row->reference_no;
                                } 
           $this->data['total_received_bill'] = count($res);
            /*
            $this->db->select('*');
            $this->db->from('book_billing');
            $this->db->where('payment_date !=', NULL);
            ...
            */
            ...
            $this->db->from('booking_gr');
            $this->db->where('bill_number <=', 0);
            $this->data['unbilledgrcount'] = $this->db->get()->num_rows();
    ...
    function dashboard_filter() {
        // Modified by Rakesh Dated: 19-03-20
            //Total GR
            
            $filter = explode("-",$this->input->post('years_period'));
            $filter= array_filter($filter);
            if(!empty($filter)){
                $startdate = convertToMySQLDate("01-04-".$filter[0]);
                $enddate   = convertToMySQLDate("31-03-".$filter[1]);
            
                $startdatetime = convertToMySQLDateTime("01-04-".$filter[0]);
                $enddatetime   = convertToMySQLDateTime("31-03-".$filter[1]);            
            ...
            if(!empty($startdate) && !empty($enddate)){
                $this->db->where('gr_date >=',$startdate);
                $this->db->where('gr_date <=',$enddate);  
            }            
            $this->db->from('booking_gr');
            $this->data['total_gr'] = $this->db->get()->num_rows();
            ...
            if(!empty($startdatetime) && !empty($enddatetime)){
                $this->db->where('added_on >=',$startdatetime);
                $this->db->where('added_on <=',$enddatetime);  
            }                        
            ...
            $this->db->where('bill_number !=', 0);
            $fresht_gr_total = $this->db->get()->row()->bill_total;
            if(!empty($fresht_gr_total)){
                $tbf = $gr_total + $fresht_gr_total;
                $this->data['total_billing_amount'] = number_format($tbf,2);                 
            }  else {
                $tbf = $gr_total;
                $this->data['total_billing_amount'] = number_format($tbf,2); 
            } 
            ...
            $bic = $this->db->get()->result();
            $bill_id_count= array();
            foreach($bic as $key => $value){
                $bill_id_count[] = $value->billing_id;
            }

            $this->db->select('*');
            $this->db->from('payment_history');
            $this->db->where('voucher_type', 'Receipt');
            $this->db->where_in('reference_no', $bill_id_count);
            
            $count_payment_for_bill = $this->db->get()->num_rows();

            $this->data['total_received_bill'] = $count_payment_for_bill;    
            ...
            $this->data['total_payment_received'] = number_format($paid_amount, 2);          
           
            $this->data['currentbillpendingamount'] = number_format(($tbf - $paid_amount),2);
           
           $this->data['rakesh'] = $bill_id_count;
