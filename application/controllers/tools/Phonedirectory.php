...
class Phonedirectory extends CI_Controller {
    ...
    function index() {
        if (!$this->flexi_auth->is_privileged('Phone Directory')) {
        ...
        $this->data['phdirs'] = $this->functions->get_all_row('tbl_phone_directory');
        $this->load->view('tools/phonedirectory/view', $this->data);
    }

    function add_contact() {
        $this->load->view('tools/phonedirectory/add', $this->data);
    }

    function edit_contact($phd_id) {
        $this->data['phdir'] = $this->functions->get_single_row('tbl_phone_directory', 'phd_id', $phd_id);

        $this->load->view('tools/phonedirectory/edit', $this->data);
    }

    function contact_details($phd_id) {
        $this->data['phdir'] = $this->functions->get_single_row('tbl_phone_directory', 'phd_id', $phd_id);
        $this->load->view('master/assets/details', $this->data);
    }

    function delete() {
        $phd_id = $this->input->post('id');
        $response = $this->functions->delete_single_row('tbl_phone_directory', 'phd_id', $phd_id);

        $activites['user_id'] = $this->flexi_auth->get_user_id();
        $activites['activity_id'] = $phd_id;
        $activites['activity_type'] = 'Phone Directory';
    ...
    function add() {
        ...
        $response = $this->functions->insert_into_table('tbl_phone_directory', $data);
