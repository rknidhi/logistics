<div class="row">
    <div class="card card-outline-primary">
        <div class="card-header">
            <h4 class="m-b-0 text-white">Update Crossing Rate</h4>
        </div>
        <div class="card-body myModal">
            <form class="m-t-10 row" id="form_validation" novalidate action="<?= $base_url ?>master/crossing_rate/update/<?= $crate->crossing_id ?>" method="POST">
                    ...
                    <div class="controls">
                        <select class="form-control" name="data[sender_branch_agent]" class="" required data-validation-required-message="Please select state">
                            ...
                            foreach ($bagents as $bagent) {
                                echo '<option value="' . $bagent->branch_agent_id . '" ' . ($bagent->branch_agent_id == $crate->sender_branch_agent ? 'selected' : '') . '>' . $bagent->branch_agent . '</option>';
                            }
                            ?>    
                        </select>
                    </div>
                ...
                <div class="form-group col-md-3 m-t-10">
                    ...
                    <div class="controls">
                        <select class="form-control" name="data[receiver_branch_agent]" class="" required data-validation-required-message="Please select state">
                            ...
                            <?php
                            foreach ($bagents as $bagent) {
                                echo '<option value="' . $bagent->branch_agent_id . '" ' . ($bagent->branch_agent_id == $crate->receiver_branch_agent ? 'selected' : '') . '>' . $bagent->branch_agent . '</option>';
                            }
                            ?>    
                        </select>
                    </div>
                    ...
                    <div class="controls">
                        <select class="form-control" name="data[to_station_id_fk]" class="" required data-validation-required-message="Please select region">
                            ...
                            foreach ($stations as $station) {
                                echo '<option value="' . $station->station_id . '" ' . ($station->station_id == $crate->to_station_id_fk ? 'selected' : '') . '>' . $station->station_name . '</option>';
                            }
                            ?> 
                        </select>
                    </div>
                ...
                <div class="form-group col-md-3 m-t-10">
                    ...
                    <div class="controls">
                        <input type="text" name="data[rate_wt]" value="<?= $crate->rate_wt ?>" class="form-control" required data-validation-required-message="This field is required"></div>
                </div>

                <div class="form-group col-md-3 m-t-10">
                    <h5>Rate (Qty.) <span class="text-danger">*</span></h5>
                    <div class="controls">
                        <input type="text" name="data[rate_qty]" value="<?= $crate->rate_qty ?>" class="form-control" required data-validation-required-message="This field is required"></div>
                </div>
