<div class="row">
    <div class="card card-outline-primary">
        ...
        <div class="card-body myModal">
            <form class="m-t-10 row" id="form_validation" novalidate action="<?= $base_url ?>master/item/update/<?= $item->item_id ?>" method="POST">
                ...
                <div class="form-group col-md-3">
                    ...
                    <div class="controls">
                        <input type="text" name="data[default_weight]" value="<?= $item->default_weight ?>" class="form-control" required data-validation-required-message="This field is required"></div>
                </div>
                ...
                <div class="form-group col-md-3">
                    ...
                    <div class="controls">
                        <select class="form-control" name="data[charging_method]" class="" required data-validation-required-message="Please select state">
                            <option value="">Select</option>
                            <?php
                            foreach ($freight_methods as $freight_method) {
                                echo '<option value="' . $freight_method->freight_method . '"' . ($freight_method->freight_method == $item->labour_method ? 'selected' : '') . '>' . $freight_method->freight_method . '</option>';
                            }
                            ?>    
                        </select>
                    </div>
                    ...
                    <div class="controls">
                        <select class="form-control" name="data[branch_agent_id_fk]" class="" required data-validation-required-message="Please select Branch">
                            ...
                            foreach ($bagents as $bagent) {
                                echo '<option value="' . $bagent->branch_agent_id . '" ' . ($bagent->branch_agent_id == $item->branch_agent_id_fk ? 'selected' : '') . '>' . $bagent->branch_agent . '</option>';
                            }
                            ?>    
                        </select>
                    </div>
