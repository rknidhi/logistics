<div class="row">
    <div class="card card-outline-primary">
        <div class="card-header">
            <h4 class="m-b-0 text-white">Update Stationary</h4>
        </div>
        <div class="card-body">
            <form class="m-t-40 row" id="form_validation" novalidate action="<?= $base_url ?>tools/stationary/update/<?= $stationary->st_id ?>" method="POST">

                <div class="form-group col-md-4">
                    <h5>Name<span class="text-danger">*</span></h5>
                    <div class="controls">
                        <input type="text" name="data[name]" value="<?= $stationary->name ?>" class="form-control" required data-validation-required-message="This field is required"></div>
                </div>
                    ...
                    <div class="controls">
                        <select name="data[type]" class="form-control select2" required>
                            ...
                            <?php foreach ($this->config->item('stationary_type') as $type): ?>
                                <option value="<?= $type ?>" <?= $type == $stationary->type ? 'selected' : '' ?>><?= $type ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                </div>
                    ...
                    <div class="controls">
                        <select name="data[company_id_fk]" class="form-control select2">
                            ...
                            foreach ($companies as $company) {
                                echo '<option value="' . $company->company_id . '" ' . ($company->company_id == $stationary->company_id_fk ? 'selected' : '' ) . '>' . $company->company_name . '</option>';
                            }
                            ?>
                        </select>
                    </div>
                    ...
                    <div class="controls">
                        <select type="text" name="data[fy_year]" class="form-control select2">
                            ...
                            for ($year = $fromyearyear; $year <= $tillyear; $year++) {
                                echo '<option value="' . ($year - 1) . '" ' . (($year - 1) == $stationary->fy_year ? 'selected' : '') . '>' . (($year - 1) . ' - ' . $year ) . '</option>';
                            }
                            ?>
                        </select>
                    </div>
