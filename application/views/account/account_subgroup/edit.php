<div class="row">
    <div class="card card-outline-primary">
        <div class="card-header">
            <h4 class="m-b-0 text-white">Update Account Sub Group</h4>
        </div>
        <div class="card-body myModal">
            <form class="m-t-40 row" id="form_validation" novalidate action="<?= $base_url ?>accounts/account_subgroup/update/<?= $account_subgroup->aasg_id ?>" method="POST">
                <div class="form-group col-md-6 m-t-10">
                    <h5>Sub Group Name <span class="text-danger">*</span></h5>
                    <div class="controls">
                        <input type="text" name="data[subgroup_name]" value="<?= $account_subgroup->subgroup_name ?>" class="form-control" required data-validation-required-message="This field is required"></div>
                </div>
                    ...
                    <div class="controls">
                        <select class="form-control" name="data[aag_id_fk]" required aria-invalid="false">
                            ...
                            foreach ($this->functions->get_all_active_row('ac_account_group') as $account_group):
                                echo '<option value = "' . $account_group->aag_id . '"' . ($account_subgroup->aag_id_fk == $account_group->aag_id ? 'selected' : '') . '>' . $account_group->group_name . '</option>';
                            endforeach;
                            ?>
                        </select>
                    </div>
