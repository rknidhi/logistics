...
<body>
                    ...
                    <div class="col-lg-12" id="voucher_details">
                        ...
                        <div class="form-group col-md-12 text-center">
                            <div class="col-sm-12">
                                ...
                                <button type="button" class="btn btn-success addnewlhcaj" name="addnew"
    ...
    <script>
$('.addnewlhcaj').click(function(){
    $.ajax({
        type: 'POST',
        dataType:"text",
        async: false,        
        url: '<?= $base_url ?>accounts/voucher/add_lhca_ajaxj',
...
$('select').on('change', function(){
    ...
    $.ajax({
            url: '<?= base_url() ?>accounts/voucher/fetch_lhc_detailsj',
            type: 'POST',
            data: ({
                id: $(this).val()
            }),
            success: function(response) {
                ...
                $('tr#'+rowno).find('td:eq(2)').html(record['a'].added_on);                
                $('tr#'+rowno).find('td:eq(3)').html(record['a'].total_freight);
                $('tr#'+rowno).find('td:eq(4)').html(record['a'].received_amount);
                $('tr#'+rowno).find('td:eq(5)').html((record['a'].total_freight)-record.received_amount);
                gettotal();
            }
        });
});
