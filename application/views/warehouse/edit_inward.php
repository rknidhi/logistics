...
<html lang="en">
    ...
    <head>
        ...
        <title><?php echo $this->config->item('title_prefix'); ?> : Broker Master</title>
                                            ...
                                            <form class="m-t-10" id="form_validation2" novalidate action="<?= $base_url ?>warehouse/searchinward" method="POST">
                                                <table width="100%" >
                                                    <tr>
                                                        ...
                                                        <td width="30%">
                                                            <input type="text" name="inward_no" value="<?= $inward->inward_no ?>" class="form-control" required>
                                                        </td>
                                            ...
                                            <form class="m-t-10" id="form_validation" novalidate method="POST">
                                                <fieldset>
                                                    ...
                                                    <table width="100%">
                                                        <tr>
                                                            ...
                                                            <td><input type="text" name="data[inward_date]" value="<?= convertToHumanDate($inward->inward_date) ?>" class="form-control singledate" required></td>
                                                            ...
                                                            <td>
                                                                <select class="form-control select2" name="data[branch_id_fk]" id="branch_id_fk" class="" required>
                                                                    ...
                                                                    foreach ($branches as $branch) {
                                                                        echo '<option value="' . $branch->branch_agent_id . '" ' . ($inward->branch_id_fk == $branch->branch_agent_id ? 'selected' : '') . '>' . $branch->branch_agent . '</option>';
                                                                    }
                                                                    ?>    
                                                                </select> 
                                                            </td>
                                                            ...
                                                            <td>
                                                                <select class="form-control select2" name="data[vendor_id_fk]" id="vendor_id_fk" class="" required>
                                                                    ...
                                                                    foreach ($vendors as $vendor) {
                                                                        echo '<option value="' . $vendor->vendor_id . '" ' . ($inward->vendor_id_fk == $vendor->vendor_id ? 'selected' : '') . '>' . $vendor->vendor_name . '</option>';
                                                                    }
                                                                    ?>    
                                                                </select>
                                                            </td>
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td><input type="text" name="data[document_date]" value="<?= convertToHumanDate($inward->document_date) ?>" class="form-control singledate" required></td>


                                                            <td><label class="control-label">Doc. Invoice No.</label></td>
                                                            <td><input type="text" name="data[doc_invoice_no]" value="<?= $inward->doc_invoice_no ?>" class="form-control" required></td>

                                                            <td><label class="control-label">Transport Name</label></td>
                                                            <td><input type="text" name="data[transport_name]" value="<?= $inward->transport_name ?>" class="form-control" required></td>
                                                        </tr>
                                                        <tr>
                                                            <td ><label class="control-label">Vehicle No</label></td>
                                                            <td><input type="text" name="data[vehicle_no]" value="<?= $inward->vehicle_no ?>" class="form-control" required></td>

                                                            <td><label class="control-label">GR No</label></td>
                                                            <td ><input type="text" name="data[gr_no]" value="<?= $inward->gr_no ?>" class="form-control" required></td>
                                                        </tr>
                                                    </table>
                                                ...
                                                <div class="form-group col-md-12 text-center">
                                                    <div class="col-sm-12">
                                                        ...
                                                        <a href="<?= $base_url ?>warehouse/inward"class="btn btn-warning btn1"><i class="fa  fa-plus"></i> New</a>
                                                        <button type="button" class="btn btn-danger btn1 delete-inward" id="<?= $inward->inward_id ?>"><i class="fa fa-trash"></i> Delete</button>
        ...
        <script>
            ...
            $('.delete-inward').click(function () {
                ...
                $.confirm({
                    ...
                    buttons: {
                        confirm: function () {
                            var url = '<?= $base_url ?>warehouse/delete_inward';
                            $.ajax({
                                url: url,
                                type: 'POST',
                                data: ({id: id}),
                                success: function (response) {
                                    ...
                                    if (res.status == 'success')
                                    {
                                        ...
                                        go_to_edit('<?= $base_url ?>warehouse/inward');
                    ...
                    success: function (response) {
                        ...
                        if (x < max_fields) { //max input box allowed
                            ...
                            var myrow = '<tr id="rowid' + x + '">'
                                    + '<td>' + x + '</td>'
                                    + '<td><select class="form-control" name="item[item_id_fk][' + x + ']" id="item_id_fk_' + x + '" required><option value="">Select</option>' + item_list + '</select></td>'
                                    + '<td><input  class="form-control input-sm" type="text" name=item[item_qty][' + x + '] id="item_qty_' + x + '" required /></td>'
                                    + '<td><select class="form-control item_ids" name="item[package_id][' + x + ']" id="package_id_' + x + '" required><option value="">Select</option>\n\
<?php
foreach ($packages as $package) {
...
<?php if ($doprint == 'TRUE') { ?>
                ...
                window.open('<?= $base_url ?>warehouse/printinward/<?= $inward->inward_id ?>', 'printwindow', settings);
<?php } ?>
        </script>
    </body>
